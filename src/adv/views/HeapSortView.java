...
public class HeapSortView extends View {
    ...
    protected HeapSortPanel panel;

    public HeapSortView(HeapSortPanel panel) {
        super();
        this.panel = panel;
    }
    ...
    private void runHeapSort() {
        panel.setLabel("Now begin to run Heap Sort.");
        repaintwait();
        setLastIndex(currentNumElements-1);
        heapSort();
    }

    //heapSort algorithm from INF2B
    private void heapSort(){
        buildHeap();
        panel.setLabel("Next run removeMax.");
        repaintwait();
        while(getLastIndex() >= 0){
            removeMax();
        }
        repaintwait();
        panel.setLabel("Heap Sort finished.");
    }


    //buildHeap algorithm from INF2B
    private void buildHeap(){
        panel.setLabel("First run buildHeap.");
        repaintwait();
    ...
    private void swapLastAndRootVertex(){
       ...
       // panel.setLabel("The max element is "+root.key+".");
        ...
        swapVertex(lastVertex(), root, lastIndex, 0);
    ...
    private void removeLastVertex(){
        panel.setLabel("Remove the max element "+arrayVertex[getLastIndex()].key+".");
        repaintwait();
        //remove the item in the tree.
        removeAny(keyLabel[getLastIndex()]);
    ...
    private AVLTreeVertex lastVertex(){
        return arrayVertex[getLastIndex()];
    }
