...
import adv.panels.AVLTreePanel;
...
public abstract class View extends GView {
    ...
    public Font ft = new Font(Font.DIALOG,Font.PLAIN,16);
    protected float zoomInFactor = 1.1f;
    protected float zoomOutFactor = 0.9f;
    protected float zoomFactor = 1f;
    protected GElementLabel nullLabel;
    ...
    protected void setUpExplainationText(String messageToDisplay) {
        ...
        explainationText = createLabel(messageToDisplay,0.17*canvas.width,(canvas.height/30));
        //explainationText = createRectangle(messageToDisplay, canvas.width / 2, (canvas.height/10)*7.5, 0.5 * canvas.width, (canvas.height/10));

    }

    protected void setUpExplainationText(String messageToDisplay, Font ft) {
        Dimension canvas = getRealSize();
        explainationText = createLabel(messageToDisplay,0.17*canvas.width,(canvas.height/30), ft);
        //explainationText = createRectangle(messageToDisplay, canvas.width / 2, (canvas.height/10)*7.5, 0.5 * canvas.width, (canvas.height/10));
    ...
    Vector2D[] createPath2(Vector2D start, Vector2D finish, int numelements) {
        int i;
    ...
    protected void AnimatePath(GElement element1, Vector2D start1, Vector2D end1, GElement element2, Vector2D start2,
        ...
        Vector2D path1[] = createPath2(start1, end1, steps);
        Vector2D path2[] = createPath2(start2, end2, steps);
        for (j = 0; j < steps; j++) {
    ...
    public DSShapeCircle createCircle2( double x, double y) {
        DSShapeCircle c = new DSShapeCircle();
        c.setRadius(22);
        c.setPosition(x, y);
        shapes.add(c);
    ...
    public GElementLabel createLabel(String label, double x, double y, Font font) {
        return createLabel(label, x, y, true, font);
    }
    ...
    public GElementLabel createLabel(String label, double x, double y, boolean draggable, Font font) {
        GElementLabel l = new GElementLabel();
        l.setPosition(x, y);
        l.setLabel(label);
        l.setDraggable(draggable);
    ...
    public void sltAllElements(){
        selectAllElements(true);
    }

    public void cancelSltAllElements(){
        selectAllElements(false);
    }
